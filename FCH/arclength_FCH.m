...
for j = 1:10
    ...
    grad = compute_RHS(gradientinit(ck), u_curr, eps, eta, Grid);
    F_curr = grad.x;
    J = grad.dx;
...
f = -eps^4 * Grid.k6 .* ck - ...
    eps^2 * Grid.k4 .* ck.*(fft(u.^3) - 1) ...
    - eps^2 * Grid.k4 .* ck.^2 .* (fft(3*u.^2) - 1) + ...
    ck .* (fft(3*u.^5 - 4*u.^3) - 1) + ...
    eta * eps^2 * Grid.k4 .* ck - ...
    eta * Grid.k2 .* ck .* (fft(u.^3) - 1);
