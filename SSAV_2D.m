function [k, tt, uu, Eu, Eu_SSAV, Em, mass, m_est_vals, t_vals, dt_vals] = ...
...
function [r, r_hat, m_est] = compute_r(u_snew, u, u_old, w, w_old, ... 
    H_snew, S, dx, dy, dt, k, Lx, Ly, nx, ny, alpha, epsilon, a, b, c)
...
r_tilde = -(b*u + c*u_old) - S*real(ifftn(reshape(k .* u_snew(:), ...
    nx, ny))) + real(ifftn( reshape(k .* rH(:), nx, ny)));
